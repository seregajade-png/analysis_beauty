          ...
          <div className="grid md:grid-cols-2 gap-4">
            {products.map((p) => (
              <button
                key={p.id}
                onClick={() => startTest(p)}
                className="card-brand p-5 text-left hover:border-brand-orange/50 hover:shadow-md transition-all group"
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-semibold text-brand-dark group-hover:text-brand-orange transition-colors">
                      {p.name}
                    </p>
                    {p.category && (
                ...
                <div className="mt-3 text-xs text-brand-orange font-medium group-hover:gap-2 transition-all">
                  Пройти тест →
                </div>
              </button>
            ))}
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="mb-6">
          ...
          <div className="section-header-orange">
            <p className="text-xs font-semibold uppercase tracking-wider opacity-80 mb-1">Тест на знание продукта</p>
            <h2 className="text-xl font-bold">{selected.name}</h2>
            {selected.price && <p className="text-sm opacity-80 mt-0.5">{selected.price.toLocaleString("ru")} ₽</p>}
          </div>
        </div>

        <div className="space-y-5">
          {QUESTIONS.map((q, i) => (
            <div key={q.key} className="card-brand p-5">
              <div className="flex items-start gap-3 mb-3">
                <span className="w-6 h-6 rounded-full bg-brand-orange text-white text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <div>
                  <p className="text-sm font-semibold text-foreground">{q.label}</p>
                  <p className="text-xs text-brand-orange mt-0.5">{q.hint}</p>
                </div>
              </div>
              <textarea
                value={answers[q.key] ?? ""}
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-brand-orange hover:bg-brand-orange-dark text-white font-semibold text-sm transition-all disabled:opacity-40 shadow-sm"
          >
            {loading ? (
    ...
    return (
      <div className="p-6 max-w-4xl mx-auto animate-fade-in">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-brand-dark">Результаты теста: {selected?.name}</h1>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          {result.strengths?.length > 0 && (
            <div className="card-brand p-4">
              <h3 className="text-sm font-semibold text-brand-green mb-2">✦ Знаете хорошо</h3>
        ...
        {result.recommendations?.length > 0 && (
          <div className="card-brand p-5">
            <h3 className="text-sm font-semibold text-brand-orange mb-3">◈ Что изучить</h3>
